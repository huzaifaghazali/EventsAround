  ...
  <div class="container-fluid text-center">
    ...
    <div class="row content">
      ...
      <div class="col-sm-8 text-left">
        ...
        <?php
        $groupMemberID = $_SESSION['onlineUserID'];
        $sql = "SELECT `groupID` FROM `tbl_group_members` WHERE `userID` = '$groupMemberID' AND `group_member_status` = 'A' ORDER BY `group_member_id` DESC";
        $result = mysqli_query($con, $sql);
        if ($result) {
          if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
              $groupID = $row['groupID'];
              $whereClause =  " WHERE `group_status` = 'A' AND `group_id` = '$groupID' ";
